...
#
trace_ctrl_msg_dic = { 2817 : 'TRACE_INFO_TYPE_LOG' ,
...
msg_dic = {'trace_ctrl' : trace_ctrl_msg_dic}
#
...
def get_msg_name(process_type, msg_type):
    if process_type in msg_dic:
        if msg_type in msg_dic[process_type]:
            return msg_dic[process_type][msg_type]

    return 'unknown_message'

def explain_msg(msg_str):
    tmp = msg_str.split(';')

    # node_name, process_type
    tmp2 = tmp[0].split(' ')
    node_name = tmp2[5]
    process_type = tmp2[6].split('[')[0]

    # msg_type
    msg_type_hex = tmp[-1][64:66] + tmp[-1][61:63]
    msg_type = int(msg_type_hex, 16)

    return process_type, node_name, msg_type



if __name__ == '__main__':
    process_type, node_name, msg_type = explain_msg(msg)
    print get_msg_name(process_type, msg_type)
